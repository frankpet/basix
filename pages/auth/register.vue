          ...
          >
            ...
            >
              <div class="d-inline-flex p-0 col-12">
                ...
                <div
                  :style="{ color: dashboard.pattern.h1 }"
            ...
            <b-card-body class="px-1">
              <form
                :style="{ color: dashboard.pattern.p }"
                @submit.prevent="register"
                class=" "
              >
                <div class="form-group row fnt-22">
                  <label
                    class="col-lg-4 col-form-label text-md-right "
                    for=""
  ...
  methods: {
    ...
    assign(user) {
      const roles = this.roles
        .filter(item => item.on_register)
        .map(item => item.name)
      this.form.roles = roles
      this.form.host = this.host
      this.form.locale = this.locale
      this.form.role = 'account'
      this.form.user = user
      this.$axios.$post('auth/assign', this.form)
    }
  }
}
</script>
